...
function mixerStore(options) {
    ...
    var defaultState = {
        collection: {},
        temp: { metas: _extends({}, defaultModel.metas), model: _extends({}, defaultModel.model) },
        fetching: false,
        loaded: false,
        error: null
    };
    ...
    function mapModels(list) {
        return list.reduce(function (prev, current) {
            var tempKey = (0, _utils.interpolate)(null, (0, _utils.extractParams)(extendedConfig.modelProps, current));
    ...
    return function reducer() {
        ...
        switch (action.type) {
            ...
            case decorate('FETCH_{uppercaseName}S_REJECTED'):
                {
                    return _extends({}, state, { fetching: false, loaded: false, end: time(), error: action.payload.error });
                }
            ...
            case decorate('INIT_{uppercaseName}'):
                {
                    ...
                    collection[key] = Object.assign({}, { metas: _extends({}, defaultModel.metas, { loaded: false }), model: _extends({}, defaultModel.model) }, previous);
            ...
            case decorate('FETCH_{uppercaseName}'):
                {
                    ...
                    if (!_previous) {
                        _collection[_key] = _extends({}, defaultModel, { metas: _extends({}, defaultModel.metas, { fetching: true, start: time() }) });
                    } else {
            ...
            case decorate('EDIT_{uppercaseName}'):
                {
                    ...
                    if (!_key4) {
                        ...
                        _collection4[_key4] = {
                            ...
                            model: Object.assign({}, _collection4[_key4].model, action.payload.data)
                        };
                        return _extends({}, state, {
            ...
            case decorate('SAVE_{uppercaseName}'):
                {
                    ...
                    var prev = action.payload.create ? state.temp : previousItem(_key5);
                    var updated = _extends({}, prev, {
                        metas: _extends({}, prev.metas, { error: null, saving: true, start: time(), saved: false })
                    });
                    if (action.payload.create) {
                        //model is new
                        return _extends({}, state, { temp: updated });
                    } else {
                        var _collection5 = update(updated, _key5);
                        return _extends({}, state, { collection: _collection5 });
                    }
                }
            ...
            case decorate('SAVE_{uppercaseName}_REJECTED'):
                {
                    ...
                    var _prev = action.payload.create ? state.temp : previousItem(_key6);
                    var _updated = _extends({}, _prev, {
                        metas: _extends({}, _prev.metas, { error: action.payload.error, end: time(), saving: false, saved: false })
                    });
                    if (action.payload.create) {
                        //model is new
                        return _extends({}, state, { temp: _updated });
                    } else {
                        var _collection6 = update(_updated, _key6);
                        return _extends({}, state, {
            ...
            case decorate('SAVE_{uppercaseName}_FULFILLED'):
                {
                    ...
                    var _prev2 = action.payload.create ? state.temp : previousItem(_key7);
                    ...
                    if (action.payload.create) {
                        ...
                        var _collection7 = update(_updated2, _key7);
                        return _extends({}, state, {
            ...
            case decorate('DELETE_{uppercaseName}'):
                {
                    ...
                    var _prev3 = previousItem(_key8);
                    var _updated3 = _extends({}, _prev3, { metas: _extends({}, _prev3.metas, { deleting: true, start: time(), fetching: true }) });
                    var _collection8 = update(_updated3, _key8);
                    return _extends({}, state, { collection: _collection8 });
                }

            case decorate('DELETE_{uppercaseName}_REJECTED'):
                {
                    var _key9 = keyGen(action.payload.params);
                    var _prev4 = previousItem(_key9);
                    var _updated4 = _extends({}, _prev4, {
                        metas: _extends({}, _prev4.metas, { error: action.payload.error, end: time(), deleting: false, fetching: false })
                    });
                    var _collection9 = update(_updated4, _key9);
                    return _extends({}, state, {
            ...
            case decorate('DELETE_{uppercaseName}_FULFILLED'):
                {
                    ...
                    var _collection10 = _extends({}, state.collection);
                    delete _collection10[_key10];
                    return _extends({}, state, {
