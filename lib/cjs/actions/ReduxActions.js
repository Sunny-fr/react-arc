...
const action_1 = require("../reducers/action");
...
class ReduxActions {
    ...
    decorate = (str) => {
        return (0, utils_1.interpolate)(str, { ...this.config, actionNamespace: this.config.name.toUpperCase() });
    };
    ...
    edit(data, params) {
        return (dispatch) => {
            dispatch({
                type: this.decorate(action_1.ACTIONS.EDIT),
                payload: { data, params },
            });
        };
    }
    ...
    fetchOne(params, props = {}, axiosOptions) {
        return (dispatch) => {
            ...
            const applyRequest = (tryNumber = 1) => {
                ...
                const actionType = this.decorate(action_1.ACTIONS.FETCH);
                dispatch({
                    type: actionType,
                    payload: {
                        params,
                ...
                return this.standAloneFetchOne(params, config, props, axiosOptions)
                    .then((response) => {
                    dispatch({
                        type: this.decorate(action_1.ACTIONS.FETCH_FULFILLED),
                    ...
                    if (error && error.code === exports.AXIOS_CANCEL_PAYLOAD.code && error.name === exports.AXIOS_CANCEL_PAYLOAD.name) {
                        dispatch({
                            type: this.decorate(action_1.ACTIONS.FETCH_CANCELLED),
                            payload: {
                    ...
                    dispatch({
                        type: this.decorate(action_1.ACTIONS.FETCH_REJECTED),
                        payload: {
                            error: errorSerializer(error),
                            params
                        },
