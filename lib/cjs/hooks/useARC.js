...
function fetchAuthorization({ ARCConfig: config, props, reduxContext, options = {} }) {
    ...
    const metaModel = (0, selectors_1.metaModelSelector)(reduxContext?.store.getState(), config, modelKey);
...
function useARC({ ARCConfig: initialConfig, props, }) {
    ...
    const modelKey = core_1.core.getKey(config, props);
    const params = core_1.core.getParams(config, props);
