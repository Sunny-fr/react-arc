...
class ARC {
    ...
    static extendConfig(ARCConfig) {
        return (0, utils_1.initializeConfig)(ARCConfig);
    }
    ...
    extractParams(props) {
        return (0, utils_1.getParams)(this.config, props);
    }
    applyHeaders(headers = {}, props) {
        ...
        return Object.keys(headers).reduce((state, header) => {
            return {
                ...state,
                [header]: (0, utils_1.interpolate)(headers[header], props),
            };
        }, {});
    }
    get({ props, params, options }) {
        const p = params || this.extractParams(props);
        return fetch((0, utils_1.interpolate)(this.config.paths.read || this.config.paths.item, p), {
    ...
    remove({ props, params, }) {
        const p = params || this.extractParams(props);
        return fetch((0, utils_1.interpolate)(this.config.paths.delete || this.config.paths.item, p), {
    ...
    create({ props, body, params, }) {
        const p = params || this.extractParams(props);
        // WARNING !!
        return fetch((0, utils_1.interpolate)(this.config.paths.create || this.config.paths.item, p), {
    ...
    update({ props, body, params, }) {
        const p = params || this.extractParams(props);
        return fetch((0, utils_1.interpolate)(this.config.paths.update || this.config.paths.item, p), {
