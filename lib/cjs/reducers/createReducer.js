...
function createReducer(options) {
    ...
    return function reducer(state, action) {
        ...
        switch (action.type) {
            ...
            case t(action_1.ACTIONS.EDIT): {
                ...
                if (!key) {
                    ...
                    collection[key] = {
                        ...
                        model: Object.assign({}, collection[key].model, sanitizeData(action.payload.data)),
                    };
                    return __assign(__assign({}, state), { collection: collection });
                }
            }
            case t(action_1.ACTIONS.SAVE): {
                var key = keyGen(action.payload.params);
            ...
            case t(action_1.ACTIONS.SAVE_REJECTED): {
                var key = keyGen(action.payload.params);
                var prev = action.payload.create ? state.temp : previousItem(key);
            ...
            case t(action_1.ACTIONS.SAVE_FULFILLED): {
                var key = keyGen(action.payload.params);
                var prev = action.payload.create ? state.temp : previousItem(key);
            ...
            case t(action_1.ACTIONS.DELETE): {
                var key = keyGen(action.payload.params);
            ...
            case t(action_1.ACTIONS.DELETE_REJECTED): {
                var key = keyGen(action.payload.params);
                var prev = previousItem(key);
            ...
            case t(action_1.ACTIONS.DELETE_FULFILLED): {
                var key = keyGen(action.payload.params);
                var collection = __assign({}, state.collection);
